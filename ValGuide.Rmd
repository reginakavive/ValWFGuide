...
```{r data-cleaning1, warning=FALSE, message=FALSE,comment = NA, tidy = TRUE, tidy.opts = list(width.cutoff = 75)}
  ...
  # Variables to be used
    vars <- c("household", "enumerator", "region",
              "T2_yield", "plot_length_T2",	"plot_width_T2",
              "T1_yield", "plot_length_T1", "plot_width_T1",
...
```{r data-transform1, tidy=TRUE, tidy.opts = list(width.cutoff = 75)}
  ...
  yield.vars <- c("household", "enumerator", "region", "T2_yield",	"plot_length_T2",	"plot_width_T2",	"riceSystem", "moisture_T2")
  pphy.data <- clean.data %>%
    select(yield.vars)
  
  # Generate Primary Productivity Harvested Yield (PPHY)
  pphy <- (pphy.data$T2_yield * (pphy.data$moisture_T2/100)) / ((pphy.data$plot_length_T2 * pphy.data$plot_width_T2)/10000)
...
`  subset_df <- dataHm.pp1[, c("household", "region","riceSystem",
                               "T2_Yha","T1_Yha","T3_Yha", "eSSR","incrSSR" )]`
...
#Variables of interest
subset_df <- dataHm.pp1[, c("household", "region","riceSystem","T2_Yha","T1_Yha","T3_Yha", 
                            "eSSR","incrSSR" )]
subset_df<-distinct(subset_df)
...
```{r phpy-pie, echo = FALSE, fig.show = 'hold', fig.cap="Effects on grain yield of T1 vs T2"}
...
`  dataHm.pp1 <- dataHm.pp1[!is.na(dataHm.pp1$T2_yield),]`
...
`  ggplot(
    ...
    scale_x_continuous(
      minor_breaks = seq(min(dataHm.pp1$T2_Yha, na.rm = TRUE), max(dataHm.pp1$T2_Yha, na.rm = TRUE), by = 0.5)  
    ) +  
    scale_y_continuous(  
      minor_breaks = seq(min(dataHm.pp1$T1_Yha, na.rm = TRUE), max(dataHm.pp1$T1_Yha, na.rm = TRUE), by = 0.5)  
    ) +  
    facet_wrap(~region) +  
    xlab("Grain yield (t/ha) of the control (T2)") +  
    ylab("Grain yield (t/ha) of the MVP (T1)") +    
    labs(title = "Yield distribution") +  
    # Apply theme as defined above
    them2`
...
ggplot(
  ...
  scale_x_continuous(
    minor_breaks = seq(  
      min(dataHm.pp1$T2_Yha, na.rm = TRUE),  
      max(dataHm.pp1$T2_Yha, na.rm = TRUE),  
      by = 0.5  
    )  
  ) +  
  scale_y_continuous(  
    minor_breaks = seq(  
      min(dataHm.pp1$T1_Yha, na.rm = TRUE),  
      max(dataHm.pp1$T1_Yha, na.rm = TRUE),  
      by = 0.5  
    )  
  ) +  
  facet_wrap(~region) +  
