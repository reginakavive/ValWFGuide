...
The graphic below shows the various stages of the data flow during the validation exercise. The highlighted section (in red) is what will be highlighted in detail in this analytics document.
...
knitr::include_graphics("./www/Logo/Validation Analytics Guidelines schema.png")
```
...
$Dry Yield (\frac{kg}{ha}) = \frac{Fresh Weight (kg) * (1-(\frac{Moisture}{100}))}{\frac{Plot Length (m) * Plot Width (m)}{10000}}$

In another use case, the required KPI may be primary fresh yield in kg per ha following this calculation: 


$Yield (\frac{kg}{ha}) = \frac{Fresh Weight (kg)}{\frac{Plot Length (m) * Plot Width (m)}{10000}}$
...
The general view indicates the simple descriptive analysis of the data to show distribution (via bar chart) and difference in the different treatments (via pie chart).
...
`  subset_df <- dataHm.pp1[, c("region", "T2_yieldha","T1_yieldha","T3_yieldha")]`
`  subset_df<-distinct(subset_df)`

`  # Group by region and calculate the averages`
`  averages_by_state <- subset_df %>%`
`    pivot_longer(!region, names_to = "Treatment", values_to = c("Yield")) %>%`
`    mutate(Treatment = sub("_yieldha", "", Treatment)) %>%`
`    group_by(region, Treatment) %>%`
`    summarise(Average = mean(Yield, na.rm = TRUE),`
`              Error = sd(Yield, na.rm = TRUE))`

`  # Plot data`
`  ggplot(`
`    averages_by_state,`
`    aes(fill=Treatment, y=Average, x=region)) +`
`    geom_bar(position="dodge", stat="identity") +`
`    geom_errorbar(aes(x = region, ymin=Average-Error, ymax=Average+Error), width=.2, position=position_dodge(.9)) +`
`    xlab("Location") +`
`  ylab(" Average grain yield (t/ha)") +`
`  scale_fill_manual(values = c("T2" = "#004080", "T1" = "#4caf50", "T3" = "#c26e60")) +`
`  # Theme as defined at the start`
`  them2`
...
`  yieldiff <- yieldiff[,c("region", "riceSystem", "eSSR")] %>%`
...
`  # Plot the data`
`  ggplot(yieldiff, aes(eSSR, ecdf, colour=riceSystem)) +`
`    geom_point() +`
...
`    summarise(Average = mean(useN, na.rm = TRUE),`
`              Error = sd(useN, na.rm = TRUE))`

`  # Plot the data`
`  ggplot(nue_by_state,`
`         aes(fill=plot, y=Average, x=Region)) +`
`    geom_bar(position="dodge", stat="identity") +`
`    geom_errorbar(aes(x = Region, ymin=Average-Error, ymax=Average+Error), width=.2, position=position_dodge(.9)) +`
`    xlab("Location") +`
